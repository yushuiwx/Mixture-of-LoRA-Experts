...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., lambda_sementic=0., lambda_class_global=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-B/32'):
        super(CLIPLoss, self).__init__()

        self.device = device
        self.model, clip_preprocess = clip.load('/mnt1/msranlpintern/wuxun/envs/clip/ViT-B-16.pt', device=self.device)

        self.clip_preprocess = clip_preprocess
        print(clip_preprocess.transforms)
        print(clip_preprocess.transforms[:2] + clip_preprocess.transforms[4:])
        self.preprocess = transforms.Compose( # Un-normalize from [-1.0, 1.0] (GAN output) to [0, 1].
        ...
        self.lambda_sementic  = lambda_sementic
        self.lambda_class_global = lambda_class_global

        self.src_text_features = None
        self.target_text_features = None
        self.angle_loss = torch.nn.L1Loss()

        self.model_cnn, preprocess_cnn = clip.load("/mnt1/msranlpintern/wuxun/envs/clip/RN50.pt", device=self.device)
        self.preprocess_cnn = transforms.Compose( # Un-normalize from [-1.0, 1.0] (GAN output) to [0, 1].
        ...
        self.img_sementic_loss = torch.nn.CosineSimilarity(dim=-1)
    ...
    def clip_image_similar_loss(self, sementic_img: torch.Tensor, target_img: torch.Tensor):
        sementic_img  = self.get_image_features(self.preprocess(sementic_img))
        target_img  = self.get_image_features(self.preprocess(target_img))
        # print("sementic_img", sementic_img.shape, "target_img", target_img.shape)
        return self.img_sementic_loss(sementic_img, target_img).mean()
        
    def forward(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str, texture_image: torch.Tensor = None, sementic_img:torch.Tensor = None):
        # sementic_img: class images
        # src_img: ins image
        clip_loss = 0.0
        loss_dict = {}

        if self.lambda_global:
            global_loss = self.lambda_global * self.global_clip_loss(target_img, [f"a {target_class}"])
            clip_loss += global_loss
            loss_dict['global_loss'] = global_loss

        if self.lambda_patch:
            patch_loss = self.lambda_patch * self.patch_directional_loss(src_img, source_class, target_img, target_class)
            clip_loss += patch_loss
            loss_dict['patch_loss'] = patch_loss

        if self.lambda_direction:
            direction_loss = self.lambda_direction * self.clip_directional_loss(src_img, source_class, target_img, target_class)
            clip_loss += direction_loss
            loss_dict['direction_loss'] = direction_loss

        if self.lambda_manifold:
            manifold_loss = self.lambda_manifold * self.clip_angle_loss(src_img, source_class, target_img, target_class)
            clip_loss += manifold_loss
            loss_dict['manifold_loss'] = manifold_loss

        if self.lambda_texture and (texture_image is not None):
            texture_loss = (self.lambda_texture * self.cnn_feature_loss(texture_image, target_img))
            clip_loss += texture_loss
            loss_dict['texture_loss'] = texture_loss
        
        # if self.lambda_sementic:
        #     # print("self.clip_image_similar_loss(sementic_img, target_img)", self.clip_image_similar_loss(sementic_img, target_img).shape)
        #     sementic_loss = self.lambda_sementic * self.clip_image_similar_loss(sementic_img, target_img)
        #     clip_loss += sementic_loss
        #     loss_dict['sementic_loss'] = sementic_loss

        # if self.lambda_class_global:
        #     class_global_loss = self.lambda_class_global * self.global_clip_loss(sementic_gene_img, [f"a {target_class}"])
        #     clip_loss += class_global_loss
        #     loss_dict['class_global_loss'] = class_global_loss
...
if __name__ == "__main__":
    ...
    # generated_img = '/mnt1/msranlpintern/wuxun/StyleHub/exp/multi_lora_exp/mixture_of_lora_experts/dreambooth_based/dreambooth/datasets/svdiff/dog_sculpture/01.png'
    generated_img = '/mnt1/msranlpintern/wuxun/StyleHub/exp/multi_lora_exp/mixture_of_lora_experts/dreambooth_based_below_gate_under_text_guidance/dreambooth/middle_results/1_gt_1_cla_img.png'
    src_img = '/mnt1/msranlpintern/wuxun/StyleHub/exp/multi_lora_exp/mixture_of_lora_experts/dreambooth_based/dreambooth/datasets/svdiff/sculpture/04.png'
    source_class = ['sculpture', 'panda']
    target_class = ['photo of a sculpture and a dog', 'photo of a panda sculpture and a dog']
    
    src_img = image_transforms(Image.open(src_img).convert('RGB'))
    generated_img = image_transforms(Image.open(generated_img).convert('RGB'))
    print(generated_img.max(), generated_img.min(), src_img.max(), src_img.min())
    print(generated_img.shape, src_img.shape)
    generated_img = generated_img.unsqueeze(0).cuda()
    src_img = src_img.unsqueeze(0).cuda()
    clip_loss = (2 - clip_loss_func(src_img, source_class, generated_img, target_class)) / 2
