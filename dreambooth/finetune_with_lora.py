        ...
        with TorchTracemalloc() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    print("previous image", image.shape)
                    vae_scale_factor = 2 ** (len(vae.config.block_out_channels) - 1)
                    image_processor = VaeImageProcessor(vae_scale_factor=vae_scale_factor)
                    image = image_processor.postprocess(image.detach().cpu(), output_type='np.array', do_denormalize=[True] * image.shape[0])[0]
                    print("image", type(image), image.shape, image.max(), image.min())
                    cv2.imwrite('./middle_results/official_{}_img.png'.format(count), np.uint8(np.clip(image * 255., 0., 255.)))
                    print("=" * 100)
                    count += 1
