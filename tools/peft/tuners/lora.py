...
class LoraConfig(PeftConfig):
    ...
    """
    number_of_lora: int = field(default=1)
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name):
        ...
        # self.number_of_lora = config['default'].number_of_lora
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = Linear(adapter_name, in_features, out_features, bias=bias, num_experts=lora_config.number_of_lora, **kwargs)
        ...
        for module in self.model.modules():
            if isinstance(module, LoraLayer):
                if module.merged:
                    ...
                    module.unmerge()
                module.active_adapte = adapter_name
                module.expert_loras.append(adapter_name)

    def _get_adapter_layers_states(self):
        for module in self.model.modules():
            if isinstance(module, LoraLayer):
                print(module)
                # print(not module.disable_adapters, module.active_adapter)
                print("=" * 100)

    def assert_all_linear(self):
        for module in self.model.modules():
            if isinstance(module, LoraLayer):
    ...
    def delete_adapter(self, adapter_name):
        ...
        for module in self.model.modules():
            if isinstance(module, LoraLayer):
                module.expert_loras.remove(adapter_name)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.expert_loras = [adapter_name]
        self.is_target_conv_1d_layer = is_target_conv_1d_layer

        # num_experts = len(self.r.keys())
        # print("num_experts", num_experts)
        # self.wg = torch.nn.Linear(num_experts * self.weight.shape[0], num_experts, bias=False)
        # self.wg.weight.data = normalized_columns_initializer(self.wg.weight.data, 0.01)

        # self.temperature_pred = torch.nn.Linear(num_experts * self.weight.shape[0], 1, bias=False)
        # nn.init.normal_(self.temperature_pred.weight, mean=0, std=0.01)
        self.temperatures = nn.Parameter(torch.tensor([1.0]))

        self.gates_record = nn.Parameter(torch.zeros([num_experts]))
        self.entropy_record = nn.Parameter(torch.zeros([1]))
    ...
    def forward(self, x: torch.Tensor, gates: torch.Tensor = None):
        ### layer-wise MoLE implementation
        B, L, D = x.shape
        previous_dtype = x.dtype
        result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
        lora_results = []
        for adapter in self.expert_loras:
            assert adapter in self.lora_A.keys()
            ...
            sub_lora_result = sub_lora_result.to(previous_dtype).unsqueeze(1)
            lora_results.append(sub_lora_result)

        lora_results = torch.cat(lora_results, dim = 1) #[B, number_experts, L, D]
        ...
        assert gates.shape[0] == B and gates.shape[1] == len(self.expert_loras), print(gates.shape)
        lora_results = lora_results * gates.unsqueeze(-1).unsqueeze(-1)

        lora_results = lora_results.sum(dim = 1) # add all lora results to one\
